        ...
        return (
          <div key={index} className='relative flex h-min gap-5'>
            ...
            <div className='space-y-1'>
              <p className='text-foreground text-base leading-none font-bold capitalize'>
                {requestState.status}
              </p>

              <time
                dateTime={requestState.created_at}
                className='text-muted-blue text-sm leading-none'
              >
                {format(
              ...
              {requestState.notes && (
                <p className='text-muted-blue text-sm'>{requestState.notes}</p>
              )}
            </div>
          </div>
        );
